    ...
    public boolean insertList(SparseArray<Ganta> list) {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if (null == entity.name || entity.name.length() == 0) stat.bindNull(2);
                else
                    stat.bindString(2, entity.name);
                if (null == entity.areaid) stat.bindNull(3);
                else
                    stat.bindLong(3, entity.areaid);
                if (null == entity.dianya || entity.dianya.length() == 0) stat.bindNull(4);
                else
                    stat.bindString(4, entity.dianya);
                if (null == entity.caizhi || entity.caizhi.length() == 0) stat.bindNull(5);
                else
                    stat.bindString(5, entity.caizhi);
                if (null == entity.xingzhi || entity.xingzhi.length() == 0) stat.bindNull(6);
                else
                    stat.bindString(6, entity.xingzhi);
                if (null == entity.taiquid) stat.bindNull(7);
                else
                    stat.bindLong(7, entity.taiquid);
                if (null == entity.huilu) stat.bindNull(8);
                else
                    stat.bindLong(8, entity.huilu);
                if (null == entity.yunxing || entity.yunxing.length() == 0) stat.bindNull(9);
                else
                    stat.bindString(9, entity.yunxing);
                if (null == entity.zuobiao || entity.zuobiao.length() == 0) stat.bindNull(10);
                else
                    stat.bindString(10, entity.zuobiao);
                if (null == entity.level) stat.bindNull(11);
                else
                    stat.bindLong(11, entity.level);
                if (null == entity.parentid) stat.bindNull(12);
                else
                    stat.bindLong(12, entity.parentid);
                if (null == entity.picquanmao || entity.picquanmao.length() == 0) stat.bindNull(13);
                else
                    stat.bindString(13, entity.picquanmao);
                if (null == entity.pictatou || entity.pictatou.length() == 0) stat.bindNull(14);
                else
                    stat.bindString(14, entity.pictatou);
                if (null == entity.picmingpai || entity.picmingpai.length() == 0) stat.bindNull(15);
                else
                    stat.bindString(15, entity.picmingpai);
                stat.bindString(16, dfu.format(new Date()));
                stat.bindString(17, dfu.format(new Date()));
                if (null == entity.lifeStatus) stat.bindLong(18, 1);
                else
                    stat.bindLong(18, entity.lifeStatus);
                if (null == entity.upgradeFlag) stat.bindLong(19, 1);
                else
                    stat.bindLong(19, entity.upgradeFlag);
                if (null == entity.areaname || entity.areaname.length() == 0) stat.bindNull(20);
                else
                    stat.bindString(20, entity.areaname);
                if (null == entity.danwei || entity.danwei.length() == 0) stat.bindNull(21);
                else
        ...
        try {
            synchronized (SYNC) {
                ...
                while (cursor.moveToNext()) {
                    ...
                    Date datecreatetime = null;
                    try {
                        if (!cursor.isNull(cOLUMNINDEXS.createtime)) {
                            datecreatetime = (Date) dfu.parse(cursor.getString(cOLUMNINDEXS.createtime));
                        }
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    entity.createtime = datecreatetime;
                    Date dateupdatetime = null;
                    try {
                        if (!cursor.isNull(cOLUMNINDEXS.updatetime)) {
                            dateupdatetime = (Date) dfu.parse(cursor.getString(cOLUMNINDEXS.updatetime));
                        }
                    } catch (ParseException e) {
                        e.printStackTrace();
                    }
                    entity.updatetime = dateupdatetime;
